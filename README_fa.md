...
- **کدِ موتور در این مخزن کپی نشده است.** این مخزن فقط پوستهی GUI را می‌دهد (AGPL-3.0). فایل‌های `aether` که اجرا می‌شوند همان باینرهای دست‌نخوردهی بالادستی‌اند، از [ریلیزهای CluvexStudio/Aether](https://github.com/CluvexStudio/Aether/releases) دریافت و با SHA-256 در [`src-tauri/aether-release.json`](src-tauri/aether-release.json) ثابت شده‌اند؛ سوره‌کدِ موتور (Corresponding Source) در مخزن بالادستی است.
