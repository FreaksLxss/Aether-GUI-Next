...
- **پنل پیشرفته**: اگر خواستید خودتان کنترل داشته باشید، یک پنل بازشونده همان گزینه‌هایی را که Aether واقعاً می‌پرسد در اختیارتان می‌گذارد:
  ...
  - **حالت اسکن**: Turbo، Balanced، Thorough، Verified یا Ironclad؛ یعنی این‌که چقدر سریع دنبال مسیر بگردد در برابر این‌که چقدر ترافیکِ کاوش تولید کند (Verified فقط دروازه‌هایی را می‌گیرد که connect-ip در آن‌ها پاسخ داده است؛ Aether 2.1.0 نام Stealth را به Verified تغییر داد)؛ Ironclad برای هر کاندیدا یک تونل واقعی باز می‌کند و یک درخواست HTTP واقعی می‌فرستد و فقط بعدش به آن اعتماد می‌کند (کندتر؛ بدون تضمین اتصال در شبکه‌ی شما)
...
- **نمایش پیشرفت زنده**: تا وقتی Aether در جست‌وجوی مسیر سالم است، برنامه زمانِ سپری‌شده و، به‌محضِ این‌که Aether بودجه‌ی اسکنش را اعلام کند، درصد و نوار پیشرفتِ واقعی نشان می‌دهد، نه فقط یک چرخنده‌ی بی‌معنی.
...
- **Psiphon بومی موتور (Aether ≥2.1.0)**: داخل خود موتور، با حالت‌های **Psiphon** (WARP → Psiphon)، **Psiphon-Reverse** (Psiphon → WARP) و **Psiphon-Only** (بدون WARP) به‌همراه شکل ترنسپورت (auto/CDN/direct) و تنظیم منطقه‌ی خروج. حالت‌های زنجیره‌ای یک شنونده‌ی SOCKS جدا (پیش‌فرض `127.0.0.1:1821`) دارند که مستقل از شنونده‌ی اصلی، Tor موتور (`1820`) و Tor تغییردهنده‌ی IP (`9050`) گزارش می‌شود؛ Psiphon-Only روی شنونده‌ی اصلی سرویس می‌دهد. Psiphon-Reverse به MASQUE روی HTTP/2 نیاز دارد. حالت‌های Only با Tor موتور ناسازگارند و reverse+reverse با هم اجرا نمی‌شوند. سه زیرسیستم Tor/Psiphon جدایند؛ فعال‌کردن هیچ‌کدام دیگری را اجرا نمی‌کند.
...
- **درون‌ریزی/برون‌برد**: کل تنظیماتان را در یک فایل ذخیره و روی یک سیستم دیگر بازیابی کنید، و داخل خودِ برنامه هم آپدیتِ جدید را چک کنید.
...
۳. **گرفتن فایل باینری Aether**
   ...
   Aether-GUI موتور ازپیش‌ساخته‌شده را از [ریلیز‌های CluvexStudio/Aether](https://github.com/CluvexStudio/Aether/releases) همراه خود قرار می‌دهد. از ریشه‌ی مخزن، ابزار مشترک پایتون را اجرا کنید (Python 3.9 یا جدیدتر؛ بدون نیاز به بسته‌ی اضافی پایتون):
...
- **Aether متعلقِ [CluvexStudio/Aether](https://github.com/CluvexStudio/Aether) است.** نام «Aether»، لوگو و شناسه‌های بصریِ آن علامت تجاریِ این پروژه است و [TRADEMARK.md](https://github.com/CluvexStudio/Aether/blob/main/TRADEMARK.md) بالادستی شرایط استفاده از آن را تعیین می‌کند؛ مجوز AGPL-3.0 روی کدِ موتور هیچ حقی برای این شناسه قائل نمی‌شود. Aether-GUI یک رابطِ گرافیکیِ مستقل و شخص ثالث است؛ هیچ وابستگی، حمایت یا تأیید رسمی از سوی CluvexStudio ندارد و هیچ‌چیز این‌جا ادعای «رسمی‌بودن» نمی‌کند.
- **کدِ موتور در این مخزن کپی نشده است.** این مخزن فقط پوسته‌ی GUI را می‌دهد (AGPL-3.0). فایل‌های `aether` که اجرا می‌شوند همان باینرهای دست‌نخورده‌ی بالادستی‌اند، از [ریلیز‌های CluvexStudio/Aether](https://github.com/CluvexStudio/Aether/releases) دریافت و با SHA-256 در [`src-tauri/aether-release.json`](src-tauri/aether-release.json) ثابت شده‌اند؛ سورس‌کدِ موتور (Corresponding Source) در مخزن بالادستی است.
- **حقوق مالکیت فکری و درخواست حذف (DMCA).** اگر فکر می‌کنید محتوایی این‌جا حق شما را نقض می‌کند (باینری موتور، برندها، مستندات یا هر چیز دیگر) از طریق GitHub با نگهدارنده تماس بگیرید یا از [فرایند DMCA گیت‌هاب](https://docs.github.com/en/site-policy/content-removal-policies/dmca-takedown-policy) استفاده کنید. موارد گزارش‌شده به‌سرعت بررسی می‌شوند و ادعایِ وارد به حذفِ محتوا یا اصلاحِ انتساب می‌انجامد.

طراحی و نگهداری Aether با CluvexStudio است. اگر به دردتان خورد، می‌توانید از [توسعه‌اش حمایت کنید](https://github.com/CluvexStudio/Aether#donate).
