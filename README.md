...
For a complete source-level map of this repository — architecture, IPC commands, state machine, ports, test inventory and known quirks — see [PROJECT_RUNDOWN.md](PROJECT_RUNDOWN.md).
...
- **Advanced panel** — for when you want control, a collapsible panel exposes the real options Aether's setup supports:
  ...
  - **Scan Mode**: Turbo, Balanced, Thorough, Verified, or Ironclad — trading route-discovery speed against how much probe traffic it generates (Verified dials only gateways measured to answer connect-ip; Aether ≥2.1.0 renamed Stealth → Verified); Ironclad opens a real tunnel through each candidate and sends a real HTTP request before trusting it (slower; not a guarantee of connectivity on your network)
...
- **Native engine Psiphon (Aether ≥2.1.0)** — built into the engine, with **Psiphon** (WARP → Psiphon), **Psiphon-Reverse** (Psiphon → WARP) and **Psiphon-Only** (no WARP) modes plus transport shape (auto/CDN/direct) and exit-region settings. Chained modes get a separate SOCKS listener (default `127.0.0.1:1821`) reported independently of the primary listener, engine Tor (`1820`) and IP Changer Tor (`9050`); Psiphon-Only serves on the primary listener. Psiphon-Reverse requires MASQUE over HTTP/2. Only-modes XOR with engine Tor, and reverse+reverse cannot run together. Three separate Tor/Psiphon subsystems — enabling one never starts another.
