...
- `src-tauri/binaries/fetch-aether.sh` pins `AETHER_VERSION` (currently `v1.9.0`) and verifies SHA256SUMS, but is **unix-only**. On Windows, download `aether-windows-x86_64.zip` + `SHA256SUMS.txt` from the pinned Aether release and extract `aether.exe` into `src-tauri/binaries/` (CI does this via the PowerShell block in `.github/workflows/build.yml`). Keep the pin in `fetch-aether.sh` in sync when bumping — CI's Windows block parses it from there. The About dialog shows the bundled core version hardcoded in `src/components/AboutDialog.tsx`.
